...
#####
#from chamfer5D import dist_chamfer_5D
#from chamfer6D import dist_chamfer_6D
#####
...
#####
'''
def calc_cd_4d(output, gt, calc_f1=False):
    # 4D chamfer distance was not provided
    # pad point clouds with 1 extra dim with values 0 to calculate 5D chamfer distance
    paddings = torch.zeros(output.size()[0], output.size()[1], 1).to(output.get_device())
    output = torch.cat([output, paddings], dim=2).to(output.get_device())
    paddings = torch.zeros(gt.size()[0], gt.size()[1], 1).to(gt.get_device())
    gt = torch.cat([gt, paddings], dim=2).to(gt.get_device())
    
    cham_loss = dist_chamfer_5D.chamfer_5DDist()
    dist1, dist2, _, _ = cham_loss(gt, output)
    cd_p = (torch.sqrt(dist1).mean(1) + torch.sqrt(dist2).mean(1)) / 2
    cd_t = (dist1.mean(1) + dist2.mean(1))
    if calc_f1:
...
def calc_cd_6d(output, gt, calc_f1=False):
    cham_loss = dist_chamfer_6D.chamfer_6DDist()
    dist1, dist2, _, _ = cham_loss(gt, output)
    cd_p = (torch.sqrt(dist1).mean(1) + torch.sqrt(dist2).mean(1)) / 2
    cd_t = (dist1.mean(1) + dist2.mean(1))
    if calc_f1:

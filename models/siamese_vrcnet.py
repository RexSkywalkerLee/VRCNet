...
class MSAP_SKN_decoder(nn.Module):
    ...
    #####
    #remove codes related to incomplete input as it is useless in rotational-invariance
    def forward(self, global_feat):
        ...
        '''
        points = torch.cat((coarse_raw, coarse_raw), 2)
    ...
    def forward(self, x, gt, is_training=True, mean_feature=None, alpha=None):
        ...
        if is_training:
            ...
            for i in range(points_copy.size()[0]):
                ...
                rotation_matrix = torch.from_numpy(rot_z @ rot_y @ rot_x).float().cuda()
        ...
        if is_training:
            ...
            #####
            #try merging prior & posterior estimation for rotational-invariance
        ...
        coarse_raw, coarse_high, coarse, fine = self.decoder(feat)

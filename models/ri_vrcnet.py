...
class SA_SKN_Res_encoder(nn.Module):
    ...
    def forward(self, features):
        ...
        pt1 = features[:, 0:self.input_size, :]
...
class MSAP_SKN_decoder(nn.Module):
    def __init__(self, num_coarse_raw, num_fps, num_coarse, num_fine, input_size=3, output_size=3, layers=[2, 2, 2, 2], knn_list=[10, 20], pk=10, points_label=False, local_folding=False):
        ...
        self.fc1 = nn.Linear(1024 + 9, 1024 + 9)
        self.fc2 = nn.Linear(1024 + 9, 1024 + 9)
        self.fc3 = nn.Linear(1024 + 9, num_coarse_raw * input_size)
        ...
        self.conv_cup2 = nn.Conv1d(self.expand_feature_size, self.output_size, 1, bias=True)
        ...
        self.conv_f2 = nn.Conv1d(self.expand_feature_size, self.output_size, 1)
    ...
    def forward(self, global_feat, point_input):
        ...
        coarse_raw = self.fc3(self.af(self.fc2(self.af(self.fc1(global_feat))))).view(batch_size, self.input_size-1, self.num_coarse_raw)
        ...
        coarse_raw = inverse_point_projection_feature(a1, a2, a3, coarse_raw)
        points = torch.cat([coarse_raw, org_points_input], 1)
        a1, a2, a3, points = point_projection_feature(points, pca=True)
        id0 = torch.zeros(batch_size, coarse_raw.shape[1], 1).cuda().contiguous()
        id1 = torch.ones(batch_size, org_points_input.shape[1], 1).cuda().contiguous()
        ids = torch.cat((id0, id1), 1)
        points = torch.cat((points, ids), 2).transpose(1,2).contiguous()
        '''
        ###
        ...
        if coarse.size()[2] < self.num_fine:
            if self.local_folding:
                ...
                center = coarse.transpose(2, 1).contiguous().unsqueeze(2).repeat(1, 1, self.num_fine // self.num_coarse, 1).view(batch_size, self.num_fine, self.output_size).transpose(2, 1).contiguous()
...
class Model(nn.Module):
    def __init__(self, args, size_z=128, global_feature_size=1024):
        ...
        self.encoder = PCN_encoder(input_size=self.input_size, output_size=global_feature_size)
        ...
        self.decoder = MSAP_SKN_decoder(num_fps=args.num_fps, num_fine=args.num_points, num_coarse=args.num_coarse, num_coarse_raw=args.num_coarse_raw, input_size=3, output_size=3, layers=layers, knn_list=knn_list, pk=args.pk, local_folding=args.local_folding, points_label=args.points_label)
    ...
    def forward(self, x, gt, is_training=True, mean_feature=None, alpha=None):
        ...
        coarse_raw, coarse_high, coarse, fine = self.decoder(feat, points)
        #coarse_raw, coarse_high, coarse, fine = self.decoder(feat, x, a1, a2, a3)
        
        coarse_raw = inverse_point_ortho_feature(a1, a2, a3, coarse_raw.transpose(1, 2).contiguous())
        coarse_high = inverse_point_ortho_feature(a1, a2, a3, coarse_high.transpose(1, 2).contiguous())
        ...
        if is_training:
            ...
            total_train_loss = loss1.mean() * 20 + loss2.mean() * 0.5 + loss3.mean() + loss4.mean() * alpha
            total_train_loss += (dl_rec.mean() + dl_g.mean() + dl_g_.mean()) * 20
            #total_train_loss = loss1.mean() * 50 + loss2.mean() * 0.5 + loss3.mean() + loss4.mean() * alpha
            #total_train_loss += (dl_rec.mean() + dl_g_.mean()) * 20 + dl_g.mean() * 50
            ...
            return {'out1': coarse_raw, 'out2': fine, 'cd_p': cd_p, 'cd_t': cd_t, 'f1': f1}

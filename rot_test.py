...
def test(device_ids):
    dataset_test = ShapeNetH5(train=False, novel_input=True, npoints=args.num_points)
    ...
    if args.save_vis:
        ...
        save_coarse_path = os.path.join(log_dir, 'pics', 'coarse')
            ...
            if args.save_vis:
                for j in range(args.batch_size):
                    ...
                    if idx in idx_to_plot:
                        ...
                        #inputs_pc = inverse_point_projection_feature(a1[j], a2[j], a3[j], inputs_cpu[j])
                        #gt_pc = inverse_point_projection_feature(a1[j], a2[j], a3[j], gt_cpu[j])
                        #completion_pc = inverse_point_projection_feature(a1[j], a2[j], a3[j], result_dict['out2'][j].cpu())
                        #plot_single_pcd(completion_pc.numpy(), os.path.join(save_completion_path, pic))
                        #plot_single_pcd(gt_pc.cpu().numpy(), os.path.join(save_gt_path, pic))
                        #plot_single_pcd(inputs_pc.cpu().numpy(), os.path.join(save_partial_path, pic))
                        plot_single_pcd(result_dict['out1'][j].cpu().numpy(), os.path.join(save_coarse_path, pic))
